...
$m365Tiles = @(
  @{ icon='&#128274;'; title='Enterprise Security & Compliance'; copy='Multi-factor authentication on every account. Conditional access that blocks sign-ins from risky countries and unmanaged devices. Defender for Endpoint on every laptop. Purview sensitivity labels. Data loss prevention that catches Social Security numbers, card numbers, and bank details before they leave your tenant. Cyber insurance ready from day one.' },
  @{ icon='&#127970;'; title='One Platform, One Bill'; copy='Professional email on your own domain. Teams for chat, video, and your business phone. 1 TB of cloud storage per person. One vendor replacing Zoom, Dropbox, antivirus, phone systems, and file shares. New person starts &mdash; one action, they have everything. Someone leaves &mdash; one click, they have nothing. One bill. One place to manage it all.' },
  @{ icon='&#129302;'; title='AI Built In'; copy='Microsoft 365 Copilot Chat &mdash; frontier AI inside your work account. Ask anything. Upload a contract and interrogate it. Word, Excel, and PowerPoint agents that build documents from a description. Enterprise data protection &mdash; your prompts and data are never used to train models. Included at no extra cost.' },
  @{ icon='&#128203;'; title='Startup IT Foundation'; copy='Domain registered to the company. A tenant built properly from day one. One identity per person across email, files, apps, and devices. MFA, conditional access, password manager, single sign-on. SPF, DKIM, and DMARC so your invoices don&rsquo;t land in spam. Costs less than a fraction of one IT hire.' },
  @{ icon='&#128737;'; title='Protecting Personal Data'; copy='Social Security numbers, card numbers, bank details, passport and driver&rsquo;s licence scans &mdash; automatically detected in email, files, and chat. Warned on, encrypted, or blocked before they leave your tenant. Know where your data is before you&rsquo;re asked &mdash; every state has breach notification law.' }
)
...
$categoryMeta = @(
  ...
  @{ key='engagement';   title='Fundraising and engagement';  blurb='Donors, grants, volunteers, and the communications that connect the suite to its community.' }
...
# --- Helper: render a tile for the home page ---
function Tile-Html($m, [switch]$m365) {
  ...
  if ($m365) {
    return @"
        <a href="/m365/" style="text-decoration:none;color:inherit;flex:1 1 280px;max-width:340px;opacity:$opacity;">
          <div style="border:1px solid #e3e3e3;border-top:4px solid $borderColor;border-radius:8px;padding:22px;height:100%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            ...
            <h3 style="margin:0 0 6px;color:#1F66B5;font-size:1.1rem;">$($m.title)</h3>
            <p style="margin:0;color:#444;font-size:.92rem;line-height:1.5;">$($m.copy)</p>
          </div>
        </a>
"@
  }
  return @"
        <a href="/$($m.slug)/" style="text-decoration:none;color:inherit;flex:1 1 280px;max-width:340px;opacity:$opacity;">
          <div style="border:1px solid #e3e3e3;border-top:4px solid $borderColor;border-radius:8px;padding:22px;height:100%;background:#fff;box-shadow:0 1px 3px rgba(0,0,0,.06);">
            <div style="font-size:2rem;line-height:1;margin-bottom:10px;">$($m.icon)</div>
            <h3 style="margin:0 0 6px;color:#1F66B5;font-size:1.1rem;">$($m.navName)</h3>
